...
class users{
	...
	public function cat_shows1()
	{		
		$query=$this->conn->query("select * from exam_list");
	   while($row=$query->fetch_array(MYSQLI_ASSOC))		
		{
	...
	public function answer2($data)
	{
		 
		 $ans=implode("",$data);
		 $hieu= $_SESSION['username'] ; 
		 ...
		 $wrong=0;
		 $no_answer=0;
		 $query=$this->conn->query("select dapan,traloi from lichsu1 where ten = '$hieu'   ");
	    while($qust=$query->fetch_array(MYSQLI_ASSOC))		
		{			
			if($qust['dapan']==$qust['traloi'])
			{
				 $right++;
			}
			elseif($qust['traloi']=="no_attempt")
			{
				 $no_answer++;
			}
			else
	...
	public function answer1($data)
	{
		 ...
		 $wrong=0;
		 $no_answer=0;
		 $query=$this->conn->query("select id,de from khach   ");
	    while($qust=$query->fetch_array(MYSQLI_ASSOC))		
		{			
			if($qust['id']==$qust['de'])
			{
